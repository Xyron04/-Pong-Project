...
void barra_mov(unsigned char *juego[]){
	uint8_t y1;
	unsigned char *bck = juego[0];
...
void menu_modo(uint8_t modo){
	if(modo == 0){
		Nokia5110_DrawFullImage((char*)modo_de_juego_1);
		
	}
	else{
		Nokia5110_DrawFullImage((char*)modo_de_juego_2);
	}
}

void menu_control(uint8_t control){
	if(control == 0){
		Nokia5110_DrawFullImage((char*)control_1);
	}
	else{
		Nokia5110_DrawFullImage((char*)control_2);
	}
}

void menu_dificultad(uint8_t opc){
	if(opc == 1){
		Nokia5110_DrawFullImage((char*)dificultad_facil);
	}
	else if(opc == 2){
		Nokia5110_DrawFullImage((char*)dificultad_medio);
	}
	else{
		Nokia5110_DrawFullImage((char*)dificultad_avanzado);
	}
}

void menu_inicio(void){

	Nokia5110_DrawFullImage((char*)inicio);
}

void gana_cpu(void){
	Nokia5110_DrawFullImage((char*)perdiste_cpu);
}

void gana_jugador(void){
	Nokia5110_DrawFullImage((char*)ganaste_cpu);
}

void ganador_j1(void){
	Nokia5110_DrawFullImage((char*)gana_j1);
}

void ganador_j2(void){
	Nokia5110_DrawFullImage((char*)gana_j2);
}

void background(void){
	Nokia5110_DrawFullImage((char*)background_pong);
}
